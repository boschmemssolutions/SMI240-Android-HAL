...
/*
 * HAL 0.0.1.0 matches driver version v1.0.0
 */
/*
 ...
 */
uint8_t HAL_ver[4] = {0, 0, 1, 0};

#define UNUSED_SENSOR_T(sensor_name)                                       \
  {                                                                        \
    .name = sensor_name, .vendor = "Bosch", .version = 1,                  \
    .handle = BSX_SENSOR_ID_INVALID, .type = 0, .maxRange = 1.0f,          \
    .resolution = 1.0f, .power = 0.0f, .minDelay = 0,                      \
    .fifoReservedEventCount = 0, .fifoMaxEventCount = 0, .stringType = "", \
    .requiredPermission = NULL, .maxDelay = 0,                             \
    .flags = SENSOR_FLAG_SPECIAL_REPORTING_MODE, .reserved = {}            \
  }
...
struct sensor_t bosch_all_sensors[SENSORLIST_INX_END] = {
    ...
    UNUSED_SENSOR_T("BOSCH proximity"),  // nonwakeup proximity, not used
    ...
    UNUSED_SENSOR_T(
        "BOSCH significant motion nonwakeup"),  // Significant Motion, not used
    ...
    {.name = "BOSCH heart rate",
     ...
     .flags = SENSOR_FLAG_ON_CHANGE_MODE,
     .reserved = {}},
    UNUSED_SENSOR_T("BOSCH tilt detector nonwakeup"),  // not used
    UNUSED_SENSOR_T("BOSCH wake gesture nonwakeup"),   // Wake Gesture, not used
    UNUSED_SENSOR_T(
        "BOSCH glance gesture nonwakeup"),  // Glance Gesture, not used
    UNUSED_SENSOR_T(
        "BOSCH pickup gesture nonwakeup"),  // Pickup Gesture, not used
    UNUSED_SENSOR_T(
        "BOSCH magnetic uncal offset"),  // magnetic field uncalibrated offset
    UNUSED_SENSOR_T(
        "BOSCH gyroscope uncal offset"),  // gyroscope uncalibrated offset
    UNUSED_SENSOR_T(
        "BOSCH power consumption nonwakeup"),  // power consumption, not used
    ...
    UNUSED_SENSOR_T(
        "BOSCH magnetic uncal offset wakeup"),  // magnetic field uncalibrated
                                                // offset wakeup
    UNUSED_SENSOR_T(
        "BOSCH gyroscope uncal offset wakeup"),  // gyroscope uncalibrated
                                                 // offset wakeup
    UNUSED_SENSOR_T("BOSCH power consumption wakeup"),  // Power Consumption
                                                        // Wakeup, not used

    {.name =
         "BOSCH Activity Recognition Sensor", /** to test AR in sensor HAL, name
                                                 is pre-defined by test app */
     .vendor = "Bosch",
     .version = 1,
...
void get_sensor_t(int32_t sensor_id, struct sensor_t **pp_sensor_t,
                  int32_t *p_list_inx) {
  uint32_t i;
  ...
  for (i = 0; i < bosch_sensorlist.list_len; i++) {
    if (NULL == bosch_sensorlist.list[i].name) {
      // NULL == name means sensor is not supported so that structure is
      // unuseful
      continue;
    } else {
...
#define DEFAULT_SENSOR_CONFIG(odr, unit, latency, len, delay)       \
  {                                                                 \
    .data_rate = odr, .latency_unit = unit, .max_latency = latency, \
    .fifo_data_len = len, .delay_onchange_Hz = delay,               \
  }

// map with enum BSX4_SENSORLIST_INX
BSX_SENSOR_CONFIG BSX_sensor_config_nonwk[SENSORLIST_INX_AMBIENT_IAQ + 1] = {

    DEFAULT_SENSOR_CONFIG(0, 0, 0, 4, 0),  // SENSORLIST_INX_GAS_RESIST
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_ACCELEROMETER
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_MAGNETIC_FIELD
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_ORIENTATION,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_GYROSCOPE,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 2,
                          5),  // SENSORLIST_INX_LIGHT,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_6_25Hz, BSX_CONFSTR_UNITms, 200, 3,
                          0),  // SENSORLIST_INX_PRESSURE,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 200, 2,
                          5),  // SENSORLIST_INX_TEMPERATURE,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 2,
                          0),  // SENSORLIST_INX_PROXIMITY,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_GRAVITY,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_LINEAR_ACCELERATION,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 9,
                          0),  // SENSORLIST_INX_ROTATION_VECTOR,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 200, 2,
                          5),  // SENSORLIST_INX_RELATIVE_HUMIDITY,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 200, 2,
                          5),  // SENSORLIST_INX_AMBIENT_TEMPERATURE,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 6,
                          0),  // SENSORLIST_INX_MAGNETIC_FIELD_UNCALIBRATED,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 8,
                          0),  // SENSORLIST_INX_GAME_ROTATION_VECTOR,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 6,
                          0),  // SENSORLIST_INX_GYROSCOPE_UNCALIBRATED,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0, 0),  // SENSORLIST_INX_SIGNIFICANT_MOTION,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0, 0),  // SENSORLIST_INX_STEP_DETECTOR,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 4,
                          5),  // SENSORLIST_INX_STEP_COUNTER,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 9,
                          0),  // SENSORLIST_INX_MAGNETIC_ROTATION_VECTOR,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 1,
                          5),              // SENSORLIST_INX_HEART_RATE,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0, 0),  // SENSORLIST_INX_TILT_DETECTOR,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0, 0),  // SENSORLIST_INX_WAKE_GESTURE,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0, 0),  // SENSORLIST_INX_GLANCE_GESTURE,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0, 0),  // SENSORLIST_INX_PICK_UP_GESTURE,
    DEFAULT_SENSOR_CONFIG(
        0, 0, 0, 6, 0),  // SENSORLIST_INX_MAGNETIC_FIELD_UNCALIBRATED_OFFSET
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 6,
                          0),  // SENSORLIST_INX_GYROSCOPE_UNCALIBRATED_OFFSET,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 3, 0),  // SENSORLIST_INX_POWER_CONSUMPTION,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 0, 2,
                          5),  // SENSORLIST_INX_AMBIENT_ALCOHOL,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 0, 2,
                          5),  // SENSORLIST_INX_AMBIENT_CO2,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 0, 2,
                          5),  // SENSORLIST_INX_AMBIENT_IAQ,
};

/*more one NULL pointer for end*/
BSX_SENSOR_CONFIG *BSX_active_confref_nonwk[SENSORLIST_INX_AMBIENT_IAQ + 1] = {
    NULL};

// map with enum BSX4_SENSORLIST_INX
// include a TIMESTAMP
BSX_SENSOR_CONFIG BSX_sensor_config_wk[SENSORLIST_INX_END -
                                       SENSORLIST_INX_WAKEUP_SIGNI_PRESSURE] = {

    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0,
                          0),  // SENSORLIST_INX_WAKEUP_SIGNI_PRESSURE,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_WAKEUP_ACCELEROMETER,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_WAKEUP_MAGNETIC_FIELD,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_WAKEUP_ORIENTATION,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_WAKEUP_GYROSCOPE,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 2,
                          5),  // SENSORLIST_INX_WAKEUP_LIGHT,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 3,
                          0),  // SENSORLIST_INX_WAKEUP_PRESSURE,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 200, 2,
                          5),  // SENSORLIST_INX_WAKEUP_TEMPERATURE,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 200, 2,
                          5),  // SENSORLIST_INX_WAKEUP_PROXIMITY,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_WAKEUP_GRAVITY,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 7,
                          0),  // SENSORLIST_INX_WAKEUP_LINEAR_ACCELERATION,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 9,
                          0),  // SENSORLIST_INX_WAKEUP_ROTATION_VECTOR,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 200, 2,
                          5),  // SENSORLIST_INX_WAKEUP_RELATIVE_HUMIDITY,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 200, 2,
                          5),  // SENSORLIST_INX_WAKEUP_AMBIENT_TEMPERATURE,
    DEFAULT_SENSOR_CONFIG(
        BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 6,
        0),  // SENSORLIST_INX_WAKEUP_MAGNETIC_FIELD_UNCALIBRATED,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 8,
                          0),  // SENSORLIST_INX_WAKEUP_GAME_ROTATION_VECTOR,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 6,
                          0),  // SENSORLIST_INX_WAKEUP_GYROSCOPE_UNCALIBRATED,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0,
                          0),  // SENSORLIST_INX_WAKEUP_SIGNIFICANT_MOTION,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0,
                          0),  // SENSORLIST_INX_WAKEUP_STEP_DETECTOR,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 4,
                          5),  // SENSORLIST_INX_WAKEUP_STEP_COUNTER,
    DEFAULT_SENSOR_CONFIG(
        BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 9,
        0),  // SENSORLIST_INX_WAKEUP_MAGNETIC_ROTATION_VECTOR,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_50Hz, BSX_CONFSTR_UNITms, 200, 1,
                          5),  // SENSORLIST_INX_WAKEUP_HEART_RATE,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0,
                          0),  // SENSORLIST_INX_WAKEUP_TILT_DETECTOR,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0,
                          0),  // SENSORLIST_INX_WAKEUP_WAKE_GESTURE,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0,
                          0),  // SENSORLIST_INX_WAKEUP_GLANCE_GESTURE,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0,
                          0),  // SENSORLIST_INX_WAKEUP_PICK_UP_GESTURE,
    DEFAULT_SENSOR_CONFIG(
        0, 0, 0, 6,
        0),  // SENSORLIST_INX_WAKEUP_MAGNETIC_FIELD_UNCALIBRATED_OFFSET
    DEFAULT_SENSOR_CONFIG(
        0, 0, 0, 6, 0),  // SENSORLIST_INX_WAKEUP_GYROSCOPE_UNCALIBRATED_OFFSET
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 0,
                          0),  // SENSORLIST_INX_WAKEUP_POWER_CONSUMPTION,
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 2, 5),  // SENSORLIST_INX_WAKEUP_ACTIVITY
    DEFAULT_SENSOR_CONFIG(0, 0, 0, 2, 0),  // SENSORLIST_INX_WAKEUP_AMBIENT_CO2,
    DEFAULT_SENSOR_CONFIG(BSX_CONFSTR_1Hz, BSX_CONFSTR_UNITms, 0, 2,
                          0),  // SENSORLIST_INX_WAKEUP_AMBIENT_IAQ,
};

/*more one NULL pointer for end*/
BSX_SENSOR_CONFIG *BSX_active_confref_wk[SENSORLIST_INX_END -
                                         SENSORLIST_INX_WAKEUP_SIGNI_PRESSURE] =
    {NULL};
...
static void encode_max_latency(int64_t max_report_latency_ns, uint16_t *value,
                               uint8_t *unit) {
  /*optimize: most used config is arranged before*/
  if (max_report_latency_ns >= 1000000LL &&
      max_report_latency_ns < 1000000000LL) {
    *value = (uint16_t)(max_report_latency_ns / 1000000LL);
    *unit = BSX_CONFSTR_UNITms;
  } else if (max_report_latency_ns >= 1000LL &&
             max_report_latency_ns < 1000000LL) {
  ...
  } else if (max_report_latency_ns >= 1000000000LL &&
             max_report_latency_ns < 16383000000000LL) {
...
int activate_configref_resort(int32_t bsx_list_index, int32_t is_enable) {
  ...
  for (i = 0; i < loop_end; i++) {
    ...
    current_bsxindex =
        bsx_listinx_base + (int32_t)(p_config_refers[i] - p_config);
 ...
 * @return: indicate if config string need to send
 */
int batch_configref_resort(int32_t bsx_list_index, int64_t sampling_period_ns,
                           int64_t max_report_latency_ns, float delay_Hz) {
  uint32_t i;
  int32_t bsx_listinx_base;
  int32_t current_bsxindex;
  BSX_SENSOR_CONFIG *p_config;
  ...
  PINFO("sampling_period_ns = %lld, max_report_latency = %lld)",
  ...
  for (i = 0; i < (*p_active_sensor_cnt); i++) {
    ...
    if (bsx_list_index == current_bsxindex) {
      ...
      p_config_refers[i]->fifo_data_len =
          max_report_latency_ns / sampling_period_ns;
      PDEBUG("i %d, fifo wm = %d)", i, p_config_refers[i]->fifo_data_len);
      return 1;
    }
  }

  p_config[bsx_list_index - bsx_listinx_base].data_rate =
      encode_datarate(sampling_period_ns);
  encode_max_latency(
  ...
  p_config[bsx_list_index - bsx_listinx_base].fifo_data_len =
      max_report_latency_ns / sampling_period_ns;
  PDEBUG("fifo wm = %d)",
         p_config[bsx_list_index - bsx_listinx_base].fifo_data_len);
...
static const BSX_INX_SPLID_MAPTBL bsx_inx_splid_maptbl[] = {
    ...
    {SENSORLIST_INX_MAGNETIC_FIELD_UNCALIBRATED,
     BSX_OUTPUT_ID_MAGNETICFIELD_RAW},
