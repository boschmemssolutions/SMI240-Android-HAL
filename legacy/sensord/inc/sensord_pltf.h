...
#define BS_LOG_FORMAT(fmt, type) \
  "[%lld]%s(%s Ln%d) " fmt "\n", GET_TIME_TICK(), type, __FILE__, __LINE__

#define LADON_LOG_FORMAT(fmt, type) "[%lld]%s" fmt "\n", GET_TIME_TICK(), type

#define PLADON(fmt, args...) \
  BS_LOG(LOG_LEVEL_LADON, LADON_LOG_FORMAT(fmt, "[LADON]"), ##args)

#define PNOTE(fmt, args...) \
  BS_LOG(LOG_LEVEL_N, BS_LOG_FORMAT(fmt, "[NOTE]"), ##args)

#define PERR(fmt, args...) \
  BS_LOG(LOG_LEVEL_E, BS_LOG_FORMAT(fmt, "[ERR]"), ##args)

#define PWARN(fmt, args...) \
  BS_LOG(LOG_LEVEL_W, BS_LOG_FORMAT(fmt, "[WARN]"), ##args)

#define PINFO(fmt, args...) \
  BS_LOG(LOG_LEVEL_I, BS_LOG_FORMAT(fmt, "[INFO]"), ##args)

#define PDEBUG(fmt, args...) \
  BS_LOG(LOG_LEVEL_D, BS_LOG_FORMAT(fmt, "[DEBUG]"), ##args)
