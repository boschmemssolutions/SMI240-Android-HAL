...
#define BSX_COMPILER_STDC_C11 \
  (1) /**< defines standard c compiler c11 version  */
#define BSX_COMPILER_STDC_C99 \
  (2) /**< defines standard c compiler c99 and higher version  */
...
typedef unsigned char
    bsx_u8_t; /**< defines bsx type for unsigned 8bit integer  */
typedef unsigned short int
    bsx_u16_t; /**< defines bsx type for unsigned 16bit integer  */
typedef unsigned long int
    bsx_u32_t; /**< defines bsx type for unsigned 32bit integer  */
...
typedef unsigned long int
    bsx_u64_t; /**< defines bsx type for unsigned 64bit integer  */
...
typedef signed short int
    bsx_s16_t;                /**< defines bsx type for signed 16bit integer  */
typedef signed int bsx_s32_t; /**< defines bsx type for signed 32bit integer  */
typedef signed long long int
    bsx_s64_t; /**< defines bsx type for signed 64bit integer  */
...
typedef union {
  ...
  struct {
    ...
    bsx_u32_t
        reserved; /**< least significant 32bit word of the long word (64bit) */
  } lw;           /**< long word (64bit) */
} bsx_data_content_t;
 ...
 *
 * @note If used within a function, the stack size may be increased tremendously
 * depending on the dimensions and frames. Use \p BSX_CREATE_FIFO_GLOBAL to
 * allocate globally in RAM instead of putting it on the stack.
 */
#define BSX_CREATE_FIFO_LOCAL(name, type, n_dims, n_frames)      \
  ...
  bsx_fifo_data_t bsx_##name = {                                 \
      0, (bsx_##name##_buffer), (type), 0U, (n_dims), (n_frames)};

/*! @brief Create a global buffer
 *
 * @note Increases the RAM consumption but does affect the stack size. Use \p
 * BSX_CREATE_FIFO_LOCAL to avoid allocating global variabes. */
#define BSX_CREATE_FIFO_GLOBAL(name, type, n_dims, n_frames)       \
  ...
  bsx_fifo_data_t bsx_##name##_g = {                               \
      0, (bsx_##name##_buffer_g), (type), 0U, (n_dims), (n_frames)};
 ...
 */
typedef enum {
  BSX_BUFFER_TYPE_FHUGE, /**< double precision floating-point as defined within
                            IEEE-754 */
  BSX_BUFFER_TYPE_S64,   /**< signed 64 bit integer */
  BSX_BUFFER_TYPE_U64,   /**< unsigned 64 bit integer */
  BSX_BUFFER_TYPE_FLONG, /**< single precision floating-point as defined within
                            IEEE-754 */
...
#define BSX_THREAD_CALIBRATION \
  (0x02U) /**< Identifier for the thread running self-calibration */
/*! @} */
