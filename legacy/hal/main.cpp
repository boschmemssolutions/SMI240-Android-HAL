...
#define MAX_EVENTS_READ 26
  ...
  for (i = 0; i < sensorsNum; ++i) {
    printf("sSensorList[%d].name = %s, handle = %d \n", i, sSensorList[i].name,
           sSensorList[i].handle);
  }
  printf("sensorsNum = %d \n", sensorsNum);

  dev = (sensors_poll_context_t *)p_hw_device_t;
  for (i = 0; i < sensorsNum; ++i) {
    dev->device.activate((sensors_poll_device_t *)dev, sSensorList[i].handle,
                         0);
  }

  for (i = 0; i < sensorsNum; ++i) {
    dev->device.batch((sensors_poll_device_1 *)dev, sSensorList[i].handle, 0,
                      10000000, 1000000000);
    dev->device.activate((sensors_poll_device_t *)dev, sSensorList[i].handle,
                         1);
  }
  ...
  while (1) {
    msg_cnt =
        dev->device.poll((sensors_poll_device_t *)dev, events, MAX_EVENTS_READ);
    for (i = 0; i < msg_cnt; ++i) {
