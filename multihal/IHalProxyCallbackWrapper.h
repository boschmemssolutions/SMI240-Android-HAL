...
/**
 * The following callback wrapper classes abstract away common functionality
 * across V2.0 and V2.1 interfaces. Much of the logic is common between the two
 * versions and this allows users of the classes to only care about the type
 * used at initialization and then interact with either version of the callback
 * interface without worrying about the type.
 */
...
public:
  ...
  virtual Return<void> onDynamicSensorsConnected(const hidl_vec<V2_1::SensorInfo>& sensorInfos) = 0;
...
public:
  ...
  ScopedWakelock createScopedWakelock(bool lock) override { return mCallback->createScopedWakelock(lock); }
...
public:
  ...
  Return<void> onDynamicSensorsConnected(const hidl_vec<V2_1::SensorInfo>& sensorInfos) override {
    return mCallback->onDynamicSensorsConnected(V2_1::implementation::convertToOldSensorInfos(sensorInfos));
  }

  void postEvents(const std::vector<V2_1::Event>& events, ScopedWakelock wakelock) override {
    return mCallback->postEvents(V2_1::implementation::convertToOldEvents(events), std::move(wakelock));
  }
};
