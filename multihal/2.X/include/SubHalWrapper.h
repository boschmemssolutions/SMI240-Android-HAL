...
public:
  ...
  virtual Return<void> getSensorsList(::android::hardware::sensors::V2_1::ISensors::getSensorsList_2_1_cb _hidl_cb) = 0;

  virtual Return<Result> setOperationMode(OperationMode mode) = 0;

  virtual Return<Result> activate(int32_t sensorHandle, bool enabled) = 0;

  virtual Return<Result> batch(int32_t sensorHandle, int64_t samplingPeriodNs, int64_t maxReportLatencyNs) = 0;

  virtual Return<Result> flush(int32_t sensorHandle) = 0;

  virtual Return<Result> injectSensorData(const Event& event) = 0;

  virtual Return<void> registerDirectChannel(const SharedMemInfo& mem, ISensors::registerDirectChannel_cb _hidl_cb) = 0;
...
public:
  ...
  virtual Return<void> getSensorsList(
    ...
    return mSubHal->getSensorsList([&](const auto& list) { _hidl_cb(convertToNewSensorInfos(list)); });
  }

  Return<Result> setOperationMode(OperationMode mode) override { return mSubHal->setOperationMode(mode); }
  ...
  Return<void> registerDirectChannel(const SharedMemInfo& mem, ISensors::registerDirectChannel_cb _hidl_cb) override {
...
public:
  ...
  Return<Result> initialize(V2_0::implementation::ISubHalCallback* callback,
                            V2_0::implementation::IScopedWakelockRefCounter* refCounter, int32_t subHalIndex) override {
    return mSubHal->initialize(new V2_0::implementation::HalProxyCallbackV2_0(callback, refCounter, subHalIndex));
  }
};
...
public:
  ...
  virtual Return<Result> injectSensorData(const Event& event) override { return mSubHal->injectSensorData_2_1(event); }

  Return<Result> initialize(V2_0::implementation::ISubHalCallback* callback,
                            V2_0::implementation::IScopedWakelockRefCounter* refCounter, int32_t subHalIndex) override {
    return mSubHal->initialize(new V2_0::implementation::HalProxyCallbackV2_1(callback, refCounter, subHalIndex));
  }
};
